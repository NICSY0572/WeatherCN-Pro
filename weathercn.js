...
const WeatherCN = (() => {
  ...
  const Providers = {
    async composite(location, args, lang) {
      ...
      if (key && needsProvider(merged) && hasScriptBudget()) {
        mergeComposite(merged, await fetchProvider("qweather", () => QWeather.fetch(location, key, lang), 1400));
      }

      if (needsProvider(merged) && hasScriptBudget()) {
  ...
  function patchHourly(root, hourly) {
    ...
    container.hours = hourly.map((item, index) => {
      const hour = item && typeof item === "object" ? item : {};
      return ({
        ...(existing[index] || {}),
  ...
  function patchNextHour(root, minutely) {
    ...
    target.minutes = minutes.slice(0, 60).map((item) => {
      const minute = item && typeof item === "object" ? item : {};
      return removeUndefined({
        forecastStart: minute.startTime,
        ...
        precipitationType: minute.precipitationType || "rain"
      });
    });
