...
const WeatherCN = (() => {
  ...
  const Http = {
    ...
    request(url, options = {}) {
      ...
      const headers = {
        ...
        "User-Agent": "WeatherCN-Pro/1.0 Loon",
        ...(options.headers || {})
      };

      return withTimeout(new Promise((resolve, reject) => {
        if (typeof $httpClient !== "undefined" && $httpClient.get) {
          $httpClient.get({ url, headers, timeout: timeoutMs / 1000 }, (error, response, body) => {
            if (error) {
              reject(error);
              return;
            }
  ...
  const Providers = {
    async composite(location, args, lang) {
      const cachedWeather = Cache.read("weather", location, WEATHER_CACHE_TTL_MS);
      const cachedAqi = Cache.read("aqi", location, AQI_CACHE_TTL_MS);
      if (cachedWeather && cachedAqi) {
        return { ...cachedWeather, airQuality: cachedAqi, cache: "hit" };
      }

      const tasks = [];
      const key = String(args.QWEATHER_KEY || args.QWEATHER_TOKEN || "").trim();
      if (key) {
        tasks.push(namedTask("qweather", QWeather.fetch(location, key, lang)));
      }
      tasks.push(namedTask("open-meteo", OpenMeteo.fetch(location)));
      if (NOAA.supports(location)) {
        tasks.push(namedTask("noaa", NOAA.fetch(location)));
      }
      tasks.push(namedTask("cma-alerts", CMA.fetchAlerts(location, lang)));
      ...
      const priority = ["qweather", "open-meteo", "noaa", "cma-alerts"];
      const byName = new Map();
      for (const result of results) {
        if (result.status === "fulfilled" && result.value && result.value.name) {
          byName.set(result.value.name, result.value.data);
        }
      }

      const merged = emptyComposite();
      for (const name of priority) {
        mergeComposite(merged, byName.get(name));
      }

      if (!merged.current && cachedWeather) {
        mergeComposite(merged, cachedWeather);
      }
      if (!merged.airQuality && cachedAqi) {
        merged.airQuality = cachedAqi;
      }

      if (hasWeatherPayload(merged)) {
        Cache.write("weather", location, withoutAirQuality(merged));
      }
      if (merged.airQuality) {
        Cache.write("aqi", location, merged.airQuality);
      }
  ...
  async function namedTask(name, promise) {
    try {
      return {
        name,
        data: await promise
  ...
  function uniqueAlerts(alerts) {
    ...
    return alerts.filter((alert) => {
      const key = `${alert.source || ""}:${alert.title || ""}:${alert.startTime || ""}`;
      if (seen.has(key)) {
        return false;
      }
      seen.add(key);
  ...
  function hasWeatherPayload(data) {
    return Boolean(data && (data.current || data.hourly.length || data.daily.length || data.minutely.length || data.alerts.length));
  }
  ...
  function withoutAirQuality(data) {
    const clone = { ...data };
    delete clone.airQuality;
    return clone;
  }
  ...
  const NOAA = {
    ...
    async fetch(location) {
      ...
      const points = await Http.json(`https://api.weather.gov/points/${lat},${lon}`, {
        headers: { "User-Agent": "WeatherCN-Pro/1.0 contact: local" }
      });
  ...
  const ApplePatch = {
    patch(originalJson, enrichment, request) {
      ...
      unlockCapabilities(json);

      if (enrichment) {
        patchCurrent(json, enrichment.current);
        patchHourly(json, enrichment.hourly);
        patchDaily(json, enrichment.daily);
        patchNextHour(json, enrichment.minutely);
        patchAirQuality(json, enrichment.airQuality);
        patchAlerts(json, enrichment.alerts);
        patchMetadata(json, enrichment, request);
      }
  ...
  function unlockCapabilities(root) {
    ...
    const visit = (node) => {
      ...
      for (const [key, value] of Object.entries(node)) {
        ...
        const featureKey = FEATURE_DATASETS.some((item) => lower.includes(item.toLowerCase()));
        ...
        if (featureKey && capabilityKey) {
          if (typeof value === "boolean") {
            ...
            node[key] = mergeDatasetArray(value);
          }
        } else if (Array.isArray(value) && /datasets|features|available/.test(lower)) {
          node[key] = mergeDatasetArray(value);
        }
        visit(node[key]);
      }
    ...
    if (typeof root.availability === "object") {
      root.availability.weatherCNPro = {
        version: VERSION,
        enabled: true,
        availableDataSets: FEATURE_DATASETS.slice()
      };
    }
  }

  function mergeDatasetArray(value) {
    const array = value.slice();
    if (array.every((item) => typeof item === "string")) {
      for (const feature of FEATURE_DATASETS) {
  ...
  function patchCurrent(root, current) {
    ...
    const target = ensureObject(root, "currentWeather");
  ...
  function patchHourly(root, hourly) {
    ...
    const container = ensureObject(root, "forecastHourly");
    const existing = Array.isArray(container.hours) ? container.hours : [];
    container.hours = hourly.map((item, index) => ({
      ...(existing[index] || {}),
      forecastStart: item.forecastStart,
      ...
      temperatureApparent: item.apparentTemperature,
  ...
  function patchDaily(root, daily) {
    ...
    const container = ensureObject(root, "forecastDaily");
    const existing = Array.isArray(container.days) ? container.days : [];
    container.days = daily.map((item, index) => ({
      ...(existing[index] || {}),
      forecastStart: item.forecastStart,
      conditionCode: item.conditionCode,
  ...
  function patchNextHour(root, minutely) {
    ...
    const target = ensureObject(root, "forecastNextHour");
    target.minutes = minutes.slice(0, 60).map((item) => removeUndefined({
      forecastStart: item.startTime,
      startTime: item.startTime,
      precipitationChance: item.precipitationChance,
      precipitationIntensity: item.precipitationIntensity,
      precipitationType: item.precipitationType || "rain"
    }));
  ...
  function patchAlerts(root, alerts) {
    ...
    target.alerts = alerts.slice(0, 20).map((alert) => removeUndefined({
      name: alert.title,
      title: alert.title,
  ...
  const RequestPatch = {
    patch(request) {
      ...
      if (dataSets) {
        const merged = unique([
          ...dataSets.split(",").map((item) => item.trim()).filter(Boolean),
          ...FEATURE_DATASETS
        ]);
