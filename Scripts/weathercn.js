...
const WeatherCN = (() => {
  ...
  const Logger = {
    configure(args) {
      state.debug = isDebugEnabled(args);
      state.dump = isDumpEnabled(args);
      state.debug = state.debug || state.dump;
    },
    info(...items) {
      if (state.debug) {
        emitLog("[WeatherCN]", items);
      }
    },
    warn(...items) {
      if (state.debug) {
        emitLog("[WeatherCN][warn]", items);
      }
    }
  };

  function isDebugEnabled(args) {
    return /^(1|true|yes|on)$/i.test(String(args.DEBUG || args.debug || args.LOG || args.log || "0").trim());
  }

  function isDumpEnabled(args) {
    return /^(1|true|yes|on)$/i.test(String(args.DUMP || args.dump || "0").trim());
  }

  function emitLog(prefix, items) {
    try {
      if (typeof console !== "undefined" && typeof console.log === "function") {
        const message = [prefix, ...items.map(logText)].filter(Boolean).join(" ");
        console.log(message);
      }
    } catch (_) {
      // Logging must never affect the weather response.
    }
  }
  ...
  const Env = {
    ...
    response() {
      ...
      state.finished = true;
      Logger.info(`$done body=${Boolean(payload && Object.prototype.hasOwnProperty.call(payload, "body"))}`);
      Logger.info("Done", payload && payload.body ? "body" : "passthrough");
  ...
  const ApplePatch = {
    patch(originalJson, enrichment, request) {
      ...
      const chinaRequest = Boolean(requestLocation && isLikelyChina(requestLocation));
      Logger.info(
        "Patch Minute gate:",
        `location=${requestLocation ? `${requestLocation.latitude},${requestLocation.longitude}` : "none"}`,
        `isChina=${chinaRequest}`,
        `minuteCount=${Array.isArray(enrichment && enrichment.minutely) ? enrichment.minutely.length : 0}`
      );
      ...
      if (enrichment) {
        ...
        if (chinaRequest) {
          ...
          Logger.info("Patch Minute skipped:", "not-china-or-no-location");
        }
  ...
  function patchNextHour(root, minutely) {
    ...
    Logger.info("Patch Minute entered:", `inputMinutes=${minutes.length}`);
    if (!minutes.length) {
      Logger.info("Patch Minute skipped:", "no-minutely-data");
      return;
    }
    if (hasMinuteForecast(root)) {
      Logger.info("Patch Minute skipped:", "native-minute-exists");
      return;
    }
    // Limitation: local tests prove the patched JSON shape, not Apple Weather UI rendering.
    const targets = objectTargets(root, "forecastNextHour", ["nextHourForecast", "nextHour", "minuteForecast"]);
    for (const container of minuteForecastContainers(root)) {
      const target = ensureObject(container, "forecastNextHour");
  ...
  function minuteForecastContainers(root) {
    const targets = [];
    const seen = new WeakSet();
    const markerKeys = ["currentWeather", "forecastHourly", "forecastDaily", "hourlyForecast", "dailyForecast"];
    const visit = (node) => {
      ...
      if (markerKeys.some((key) => Object.prototype.hasOwnProperty.call(node, key))) {
        targets.push(node);
      }
  ...
  async function handleRequest() {
    ...
    Logger.info("Matched URL", request.url || "");
    if (request.headers && request.headers["X-WeatherCN-Pro"]) {
      Logger.info("Done request reason=weathercn-header");
      Env.done({});
      return;
    }
    Logger.info("Done request patched=true");
    Env.done(RequestPatch.patch(request));
  }

  async function handleResponse() {
    const request = Env.request();
    const isV2Weather = isV2WeatherRequest(request);
    Logger.info("Matched URL", request.url || "");
    if (isV2Weather) {
      Logger.info("v2 weather response script hit");
    }
    const response = Env.response();
    const bodyInfo = responseBodyInfo(response);
    Logger.info(`body type=${typeof response.body}`);
    Logger.info(`body length=${bodyInfo.length}`);
    Logger.info(`content-encoding=${headerValue(response.headers, "content-encoding") || "none"}`);
    Logger.info(`content-type=${headerValue(response.headers, "content-type") || "unknown"}`);
    Logger.info("Body length", bodyInfo.length);
    Logger.info("Decoded OK", bodyInfo.decoded, bodyInfo.binary ? "binary=true" : "binary=false");
    const originalBody = bodyInfo.text;
    if (!originalBody || typeof originalBody !== "string") {
      dumpRawResponse(request, response, bodyInfo, false, null, "no-body");
      Logger.info("JSON parsed=false reason=no-body");
      if (isV2Weather) {
        Logger.info("forecastNextHour patched=false reason=no-body");
      }
      Logger.info("Done original reason=no-body");
      Env.done({});
      return;
    }
    if (!looksLikeJson(originalBody)) {
      dumpRawResponse(request, response, bodyInfo, false, null, "unsupported-body-format");
      Logger.info("Decoded OK", false, "reason=non-json-or-protobuf");
      Logger.info("Patch Failed", "unsupported-body-format");
      if (isV2Weather) {
        Logger.info("forecastNextHour patched=false reason=unsupported-body-format");
      }
      Env.done({});
      return;
    }
    ...
    try {
      ...
      Logger.info("JSON parsed=true");
    } catch (error) {
      dumpRawResponse(request, response, bodyInfo, false, null, `parse-error:${error && error.message}`);
      Logger.info("JSON parsed=false reason=parse-error", error && error.message);
      if (isV2Weather) {
        Logger.info("forecastNextHour patched=false reason=parse-error");
      }
      Logger.info("Done original reason=parse-error");
      Env.done({});
      return;
    }
    dumpRawResponse(request, response, bodyInfo, true, originalJson, "");
    if (isV2Weather) {
      const weatherContainers = minuteForecastContainers(originalJson);
      Logger.info(`weather container found=${weatherContainers.length > 0}`);
      Logger.info(`weather container count=${weatherContainers.length}`);
    }
    logWeatherJsonKeys("Before", originalJson, request.url || "");

    if (isAvailabilityRequest(request)) {
      try {
        Logger.info("Patch Begin", "availability");
        const patched = cloneJson(originalJson);
        safePatch("availability", () => unlockCapabilities(patched));
        logWeatherJsonKeys("After", patched);
        Logger.info("Patch Success", "availability");
        Env.done({
          body: JSON.stringify(patched),
          headers: scrubHeaders(response.headers)
        });
      } catch (error) {
        Logger.warn("availability patch failed", error && error.message);
        Logger.info("Patch Failed", "availability");
        Env.done({});
      }
      return;
    }
    ...
    Logger.info("Location parsed", location ? `${location.latitude},${location.longitude}` : "none");
    let enrichment = null;
    if (location) {
      try {
    ...
    Logger.info(
      "Enrichment payload",
      `has=${hasAnyPayload(enrichment)}`,
      `minuteCount=${Array.isArray(enrichment && enrichment.minutely) ? enrichment.minutely.length : 0}`
    );
    if (!hasAnyPayload(enrichment)) {
      if (isV2Weather) {
        Logger.info("forecastNextHour patched=false reason=no-enrichment");
      }
      Logger.info("Done original reason=no-enrichment");
      Env.done({});
      return;
    }

    try {
      Logger.info("Patch Begin", "weather");
      const patched = ApplePatch.patch(originalJson, enrichment, request);
      logWeatherJsonKeys("After", patched);
      if (isV2Weather) {
        Logger.info(`forecastNextHour patched=${hasMinuteForecast(patched)}`);
      }
      Logger.info("JSON stringify start");
      const patchedBody = JSON.stringify(patched);
      Logger.info("Patch Success", "weather");
      Logger.info("Done patched=true");
      Env.done({
        body: patchedBody,
        headers: scrubHeaders(response.headers)
      });
      ...
      Logger.info("Patch Failed", "weather");
      if (isV2Weather) {
        Logger.info("forecastNextHour patched=false reason=patch-error");
      }
      Logger.info("Done original reason=patch-error");
      Env.done({});
    }
  }

  function isV2WeatherRequest(request) {
    return /weatherkit\.apple\.com\/api\/v2\/weather/.test(String(request && request.url || ""));
  }

  function isAvailabilityRequest(request) {
    return /weatherkit\.apple\.com\/api\/v[12]\/availability/.test(String(request && request.url || ""));
  }
  ...
  function dumpRawResponse(request, response, bodyInfo, jsonParsed, json, reason) {
    if (!state.dump) {
      return;
    }
    const kind = dumpKind(request);
    if (!kind) {
      return;
    }
    const region = dumpRegion(request, json);
    const body = bodyInfo && typeof bodyInfo.text === "string" ? bodyInfo.text : "";
    const chunks = jsonParsed ? chunkText(body, 1800) : [];
    const keyBase = `${CACHE_PREFIX}.dump.${region}.${kind}`;
    const meta = removeUndefined({
      region,
      kind,
      url: request && request.url,
      bodyLength: bodyInfo && bodyInfo.length,
      decoded: bodyInfo && bodyInfo.decoded,
      binary: bodyInfo && bodyInfo.binary,
      jsonParsed,
      reason,
      contentEncoding: headerValue(response && response.headers, "content-encoding") || "none",
      contentType: headerValue(response && response.headers, "content-type") || "unknown",
      timestamp: new Date().toISOString(),
      chunks: chunks.length
    });

    Logger.info(`[Dump] request.url=${request && request.url ? request.url : ""}`);
    Logger.info(`[Dump] target=${region}.${kind}`);
    Logger.info(`[Dump] body length=${bodyInfo && bodyInfo.length}`);
    Logger.info(`[Dump] decoded=${Boolean(bodyInfo && bodyInfo.decoded)} binary=${Boolean(bodyInfo && bodyInfo.binary)}`);
    Logger.info(`[Dump] content-encoding=${meta.contentEncoding}`);
    Logger.info(`[Dump] content-type=${meta.contentType}`);
    Logger.info(`[Dump] JSON.parse=${jsonParsed}`);
    if (!jsonParsed) {
      Logger.info(`[Dump] parse failed reason=${reason || "unknown"}`);
      return;
    }

    writeDumpChunks(keyBase, body, chunks, meta);
    Logger.info(`[Dump] persistent-key=${keyBase}`);
    Logger.info(`[Dump][BEGIN] ${region} ${kind} complete-json chunks=${chunks.length}`);
    chunks.forEach((chunk, index) => {
      Logger.info(`[Dump][${region}.${kind}][${index + 1}/${chunks.length}] ${chunk}`);
    });
    Logger.info(`[Dump][END] ${region} ${kind}`);
  }

  function dumpKind(request) {
    const url = String(request && request.url || "");
    if (/weatherkit\.apple\.com\/api\/v1\/availability/.test(url)) {
      return "availability";
    }
    if (/weatherkit\.apple\.com\/api\/v2\/weather/.test(url)) {
      return "weather";
    }
    return "";
  }

  function dumpRegion(request, json) {
    const countryCode = String(findFirstValue(json, ["countryCode", "country", "country_code"]) || "").toUpperCase();
    const location = Location.fromRequest(request);
    if (countryCode === "CN" || (location && isLikelyChina(location))) {
      return "China";
    }
    if (isScotlandLocation(location)) {
      return "Scotland";
    }
    if (["GB", "UK", "GBR"].includes(countryCode)) {
      return "ScotlandOrUK";
    }
    return "Unknown";
  }

  function isScotlandLocation(location) {
    if (!location) {
      return false;
    }
    const latitude = Number(location.latitude);
    const longitude = Number(location.longitude);
    return latitude >= 54.5 && latitude <= 61.5 && longitude >= -8.8 && longitude <= -0.5;
  }
  ...
  function findFirstValue(root, keys) {
    ...
    const visit = (node) => {
      ...
      for (const [key, value] of Object.entries(node)) {
        if (wanted.has(key.toLowerCase()) && value != null && typeof value !== "object") {
          return value;
        }
      }
      for (const value of Object.values(node)) {
  ...
  function writeDumpChunks(keyBase, body, chunks, meta) {
    try {
      Env.write(JSON.stringify(meta), `${keyBase}.meta`);
      Env.write(body, `${keyBase}.body`);
      Env.write(String(chunks.length), `${keyBase}.chunks`);
      chunks.forEach((chunk, index) => {
        Env.write(chunk, `${keyBase}.chunk.${index + 1}`);
      });
    } catch (error) {
      Logger.warn("dump write failed", keyBase, error && error.message);
    }
  }

  function responseBodyInfo(response) {
    if (!response) {
      return { text: "", length: 0, decoded: false, binary: false };
    }
    if (typeof response.body === "string") {
      return { text: response.body, length: response.body.length, decoded: response.body.length > 0, binary: false };
    }
    const raw = response.bodyBytes || response.body;
    const text = bytesToText(raw);
    const length = byteLength(raw, text);
    return { text, length, decoded: Boolean(text), binary: Boolean(raw) };
  }
  ...
  function toByteArray(value) {
    ...
    if (typeof ArrayBuffer !== "undefined" && value instanceof ArrayBuffer && typeof Uint8Array !== "undefined") {
      return new Uint8Array(value);
    }
    if (Array.isArray(value)) {
      return typeof Uint8Array !== "undefined" ? new Uint8Array(value) : value;
    }
    if (value.buffer && typeof Uint8Array !== "undefined") {
      return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength || value.length || 0);
    }
    return null;
  }

  function byteLength(raw, text) {
  ...
  function logWeatherJsonKeys(stage, root, url = "") {
    if (!state.debug) {
      return;
    }
    if (url) {
      Logger.info(`[${stage}] URL=${url}`);
    }
    if (!root || typeof root !== "object") {
      Logger.info(`[${stage}] rootKeys=<non-object>`);
      return;
    }
    const keysOf = (node) => (node && typeof node === "object" && !Array.isArray(node))
      ? Object.keys(node).slice(0, 80).join(",")
      : "";
    const exists = (node) => Boolean(node && typeof node === "object");
    const response = exists(root.response) ? root.response : null;
    const datasets = exists(root.datasets) ? root.datasets : (response && exists(response.datasets) ? response.datasets : null);
    const rootForecast = exists(root.forecastNextHour) ? root.forecastNextHour : null;
    const rootNextHour = exists(root.nextHourForecast) ? root.nextHourForecast : null;
    const responseForecast = response && exists(response.forecastNextHour) ? response.forecastNextHour : null;
    const responseNextHour = response && exists(response.nextHourForecast) ? response.nextHourForecast : null;
    const datasetsForecast = datasets && exists(datasets.forecastNextHour) ? datasets.forecastNextHour : null;
    const datasetsNextHour = datasets && exists(datasets.nextHourForecast) ? datasets.nextHourForecast : null;

    Logger.info(`[${stage}] rootKeys=${keysOf(root)}`);
    Logger.info(`[${stage}] datasetsKeys=${keysOf(datasets)}`);
    Logger.info(`[${stage}] forecastNextHour exists=${Boolean(rootForecast || responseForecast || datasetsForecast)}`);
    Logger.info(`[${stage}] nextHourForecast exists=${Boolean(rootNextHour || responseNextHour || datasetsNextHour)}`);
    Logger.info(`[${stage}] forecastNextHour keys=${keysOf(rootForecast)}`);
    Logger.info(`[${stage}] nextHourForecast keys=${keysOf(rootNextHour)}`);
    Logger.info(`[${stage}] response.forecastNextHour keys=${keysOf(responseForecast)}`);
    Logger.info(`[${stage}] response.nextHourForecast keys=${keysOf(responseNextHour)}`);
    Logger.info(`[${stage}] datasets.forecastNextHour keys=${keysOf(datasetsForecast)}`);
    Logger.info(`[${stage}] datasets.nextHourForecast keys=${keysOf(datasetsNextHour)}`);

    for (const key of Object.keys(root).slice(0, 30)) {
      const value = root[key];
      if (exists(value)) {
        Logger.info(`[${stage}] secondLevel.${key}=${keysOf(value)}`);
      }
    }
  }
  ...
  async function main() {
    Args.parse();
    Logger.info("Plugin loaded", VERSION);
    Logger.info("Entry", Env.isRequest() ? "http-request" : (Env.isResponse() ? "http-response" : "generic"));
    if (Env.isRequest()) {
      await handleRequest();
      return;
    }
    ...
    Logger.info("Done original reason=no-request-response");
    Env.done({});
  }
